        ...
        "columns": [
            { "defaultContent": "" },
            {
                ...
                render: function (data, type, row) {
                    console.log("llamo a render y devuelve => ", data)
...
$('#tabla_empleado').on('click', '.editar', function () {
    ...
    document.getElementById('selImg').value = data.empl_fotoperfil;
})
...
function Registrar_Empleado() {
    ...
    let img = document.getElementById('selImg').value;
    if (nro.length == 0 || nom.length == 0 || apepa.length == 0 || apema.length == 0 || fnac.length == 0 || movil.length == 0 || dire.length == 0 || email.length == 0 || img.length == 0) {
...
function Modificar_Empleado() {
    ...
    let img = document.getElementById('selImg').value;
    if (id.length == 0 || esta.length == 0 || nro.length == 0 || nom.length == 0 || apepa.length == 0 || apema.length == 0 || fnac.length == 0 || movil.length == 0 || dire.length == 0 || email.length == 0 || img.length == 0) {
        return Swal.fire("Mensaje de Advertencia", "Tiene campos vacios", "warning");
    }
